...
function loginUser(req, res) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            const queryResultUser = yield pool.query("SELECT id, nombre, apellido, email, password FROM users WHERE email = $1", [email]);
            ...
            const queryResultRoles = yield pool.query("SELECT r.id, r.nombre FROM roles r INNER JOIN roles_users ru ON r.id = ru.id_rol WHERE ru.id_user = $1", [userId]);
            const rol = queryResultRoles.rows[0].nombre;
            const idRol = queryResultRoles.rows[0].id;
            const token = jsonwebtoken_1.sign({ id: userId, nombre, apellido, email, rol, id_rol: idRol }, process.env.SECRETKEY);
